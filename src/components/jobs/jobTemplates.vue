<template>
    <div class="job-template-container">
        <template v-if="jobTemplateData.length">
            <div class="job-item" v-for="(item,i) in jobTemplateData" :key="i">
                ...
                <div class="job-item-b">
                    ...
                    <div class="job-item-btns">
                        <el-button type="primary" @click="postJobFromTemplate(item)">Post Job</el-button>
                    </div>
                </div>
                <div class="job-item-more-actions">
                    <el-dropdown :hide-on-click="false" trigger="click">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item>
                                    <el-button link @click="editJobTemplate(item)">
                                        Edit
                                    </el-button>
                                </el-dropdown-item>
                                <el-dropdown-item>
                                    <el-popconfirm
                                        ...
                                        width="auto"
                                        @confirm="deleteJobTemplate(item.id)"
                                    >
                                        <template #reference>
        ...
        <template v-else>
            <div class="job-template-empty-tips">
                You have zero job post template. Create a Template from <el-button link @click="turnManualPosting()">Manual Posting</el-button>
            </div>
        </template>
...
defineProps(['jobTemplateData'])

const emit = defineEmits(['postJob','edit','delete','turnManualPosting'])
...
.upgrade-job-template-item-t {
    display: flex;
    flex-direction: row;
}

.upgrade-job-template-item-t-l {
    margin-right: 9px;
}

.upgrade-job-template-avatar {
    width: 48px;
    height: 48px;
}

.upgrade-job-template-item-label {
    font-family: 'Inter';
    font-style: normal;
    font-weight: 600;
    font-size: 16px;
...
.upgrade-job-template-item-tips {
    font-family: 'Inter';
    font-style: normal;
    font-weight: 400;
    font-size: 12px;
