    ...
    await USER_INFO_BY_TOKEN_V2(params).then(res => {
        ...
        if (res.code == 200) {
            ...
            if (companyInfo.address) {
                basicForm.address = companyInfo.address;
            }
            ...
            if (companyInfo.country_code) {
                contactForm.country_code = companyInfo.country_code;
            }

            if (companyInfo.work_phone) {
                contactForm.work_phone = companyInfo.work_phone;
            }

            if (companyInfo.profile_photo) {
                contactForm.profile_photo = companyInfo.profile_photo
            }
            ...
            if (companyInfo.desc) {
                mediaForm.desc = companyInfo.desc;
            }
        ...
        success(file) {
            ...
            if (isInChina === 'yes') {
                ...
                UPLOAD_BY_ALI_OSS(formData).then(res => {
                    ...
                    if (res.code == 200) {
                        let myFileUrl = res.data[0]['file_url'];
                        contactForm.profile_photo = myFileUrl
                        uploadLoadingStatus.value = false;
                    }
                }).catch(err => {
                    console.log(err)
